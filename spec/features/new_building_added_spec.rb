...
context "check to see if a building can be created succesfully" do
    ...
    it "creates a building when you clcik Create create builing" do
  ...
  context "Building is not created if all required fields are not filled" do
    ...
    it "deos not create a building if there is not a vaild state provided" do
      building = FactoryGirl.create(:building)
      visit 'buildings/new'
      ...
      expect(page).to have_content "Please add valid name, city, state, and zip code" 
      expect(page).to have_button "Create Building" 
